...
from smac.optimizer.acquisition import AbstractAcquisitionFunction, LogEI
...
class CustomSMAC4BO(SMAC4AC):
    """
    Facade to use SMAC for BO using a GP

    Only change to original SMAC4BO: Can specifiy GP seed.
    ...
    * The random fraction is set to ``0.08447232371720552``, it was ``0.0`` before.
    ...
    def __init__(self, model_type: str = "gp_mcmc", gp_seed: int = None, **kwargs: typing.Any):
        """
        Constructor
        ...
        ----------
        gp_seed : int
            Seed for Gaussian Process. The default is None. If it is None, a random seed is generated via the random
            generator.
        """
...
class SimpleLCB(AbstractAcquisitionFunction):
    def __init__(self, model: BaseEPM, par: float = 1.0):
        ...
        par : float, default=0.0
            ...
            acquisition function.
        """
        super(SimpleLCB, self).__init__(model)
        self.long_name = "Lower Confidence Bound (Simple)"
        self.par = par  # TODO lower or higher than zero?
